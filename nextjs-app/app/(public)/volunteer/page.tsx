...
export default async function VolunteerPage() {
  ...
  const bannerTextColor = (settings as any)?.volunteerBannerTextColor || "#FFFFFF";
  const bannerBadge = (settings as any)?.volunteerBannerBadge || "Get Involved";
  const bannerTitle = (settings as any)?.volunteerBannerTitle || "Volunteer With Us";
  const bannerSubtitle =
    (settings as any)?.volunteerBannerSubtitle ||
    "Your time, skills, and passion can change lives. Join our community of 500+ volunteers working to build a better Bangladesh.";

  return (
    <div className="pt-20">
      {/* Hero */}
      {settings?.volunteerBannerImage ? (
        <section className="relative bg-gradient-to-br from-green to-primary min-h-[180px] md:min-h-[220px] overflow-hidden">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={settings.volunteerBannerImage} alt="" className="w-full h-auto block" />
          <div
            className="absolute inset-0"
            style={{
              backgroundColor: (settings as any).volunteerBannerOverlayColor || "#0F3D8C",
              opacity: ((settings as any).volunteerBannerOverlayOpacity ?? 80) / 100,
            }}
          />
          ...
          <div className="flex items-center justify-center gap-2 mt-6 text-sm" style={{ color: bannerTextColor, opacity: 0.6 }}>
            ...
            <span>/</span>
            <span style={{ opacity: 1 }}>Volunteer</span>
          </div>
        </div>
          </div>
        </section>
      ) : (
        <section className="relative bg-gradient-to-br from-green to-primary min-h-[280px] md:min-h-[360px] lg:min-h-[420px] flex items-center py-16">
          <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <span className="inline-block bg-white/20 text-xs font-semibold uppercase tracking-widest px-4 py-1.5 rounded-full mb-6" style={{ color: bannerTextColor }}>
            {bannerBadge}
          </span>
